...
const uint16_t ledCount = DOTSUU; 
...
#define ICM_SPI_FREQ    20000000  // 10MHz
...
void updateLEDPattern() {
  ...
  if (local_swing_direction) {
    // 右→左の振り：0列目から順番に増加
    if (((uint16_t)(display_angle) >= MOJISUU_MIN) && 
        ((uint16_t)(display_angle) <  MOJISUU_MAX) ) {
      
      idx = (uint16_t) ( ((float)MOJISUU/(MOJISUU_MAX-MOJISUU_MIN))*(display_angle-MOJISUU_MIN) );
    ...
    if (((uint16_t)(display_angle) >= MOJISUU_MIN) && 
      ...
      // 角度を列インデックスにマッピング
      uint16_t angle_based_idx = (uint16_t)  ( ((float)MOJISUU/(MOJISUU_MAX-MOJISUU_MIN))*(display_angle-MOJISUU_MIN) );

...
{
  float t_angle ;
  static uint32_t premic ; 
  uint32_t curmic, i  ; 
    
  while(1){  
    curmic = micros();
    led_micros = curmic - premic; 
    premic = curmic ;
...
//  xTaskCreatePinnedToCore(  telnetserver, "TELNET_TASK", 4096, NULL, 1, NULL, 1);
  xTaskCreatePinnedToCore(  telnetserver, "TELNET_TASK", 4096, NULL, 1, NULL, 0); // 20221023
#endif 

  // ウォッチドッグ停止
  disableCore0WDT();
//  disableCore1WDT(); 
  xTaskCreatePinnedToCore(  gyro_i2c, "I2C_TASK", 8192, NULL, 1, NULL, 0);
  xTaskCreatePinnedToCore(  led_blink_int, "LED_TASK", 4096, NULL, 1, NULL, 1);
...
void loop() 
{
  ...
  char sstr[4096];
  char stmp[256];
  sprintf(sstr, "del_t, %d, led_t, %d, angle, %lf, angvel, ", del_micros, led_micros, angle);
...
//  if(Serial) Serial.println(angvel[0],3);
  ...
  if( (cur_millis - pre_millis) > 200 ){
    ...
    blinkcnt++; 
//    if(Serial) Serial.println(blinkcnt%2);
